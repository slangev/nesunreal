...
NesMMC6::NesMMC6()
{
    BankSelect = std::make_unique<NesMMC6BankSelectRegister>();
    ChrBankData.resize(6);
    PrgBankData.resize(2);
}

NesMMC6::NesMMC6(std::shared_ptr<std::vector<uint8>> PrgRomMemory, std::shared_ptr<std::vector<uint8>> PrgRamMemory, std::shared_ptr<std::vector<uint8>> ChrRomMemory, std::shared_ptr<std::vector<uint8>> ChrRamMemory, bool bBatteryBacked)
{
...
uint8 NesMMC6::Read(unsigned short Address)
{
    ...
    if (Address >= 0x0000 && Address <= 0x1FFF) 
    {
        if(BankSelect->GetCHRROMBankMode() == 0)
        {
            ...
            if(Address >= 0x0000 && Address <= 0x07FF)
            {
                ...
                Index = ((BANK_SIZE_1KB * BankNumber | (Address & 0x07FF)));
            }
            ...
            else if(Address >= 0x1000 && Address <= 0x13FF)
            {
                ...
                Index = ((BANK_SIZE_1KB * BankNumber | (Address & 0x03FF)));
            }
...
void NesMMC6::Write(unsigned short Address, uint8 Data)
{
    OpenBusData = Data;
    ...
    if(Address >= 0x0000 && Address < 0x2000) 
    {
        if(ChrRamMemory) 
        {
            ...
            uint32 Index = ((BANK_SIZE_8KB * bankNumber | (Address & 0x1FFF))) & (ChrRamMemory->size() - 1);
