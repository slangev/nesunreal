...
NesMMC4::NesMMC4(shared_ptr<vector<uint8>> PrgRomMemory, shared_ptr<vector<uint8>> PrgRamMemory, shared_ptr<vector<uint8>> ChrRomMemory, shared_ptr<vector<uint8>> ChrRamMemory, bool bBatteryBacked)
{
...
uint8 NesMMC4::GetMirrorMode() 
{
    return MirrorMode+2; // The caller(NesPPUMMU) is expecting vertical to be 2 or horizontal to by 3
}

bool NesMMC4::GetIRQRequested()
{
    return false;
}

uint8 NesMMC4::Read(unsigned short Address) 
{
    ...
    */
    if (Address >= 0x0000 && Address <= 0x1FFF) 
    {
        ...
        else if(Address == 0x0FE8 && Address <= 0xFEF)
        {
            Latch0 = 0xFE;
        }
    ...
    else if(Address >= 0x8000 && Address <= 0xFFFF) 
    {
        ...
        {
            uint BankNumber = (PrgRomMemory->size() / ROM_BANK_SIZE_16KB) - 1; // (-1) : the last bank
            Index = ((ROM_BANK_SIZE_16KB * BankNumber | (Address & 0x3FFF))) & (PrgRomMemory->size() - 1);
        }
...
void NesMMC4::Write(unsigned short Address, uint8 Data) 
{
